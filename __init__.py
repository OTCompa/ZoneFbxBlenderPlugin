...
def add_mix_node(material):
    ...
    add_and_swap_nodes(tree, main_texture_node, main_node.inputs['Base Color'], material['BlendDiffuse'])
...
def add_mix_node_normal(material):
    ...
    add_and_swap_nodes(tree, main_texture_node, normal_map_node.inputs['Color'], material['BlendNormal'])

def add_mix_node_specular(material):
    tree = material.node_tree
    main_node = tree.nodes['Principled BSDF']
    main_texture_node = main_node.inputs['Specular IOR Level'].links[0].from_node
    add_and_swap_nodes(tree, main_texture_node, main_node.inputs['Specular IOR Level'], material['BlendSpecular'])
